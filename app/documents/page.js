...
export default function Dashboard() {
      ...
      <>
        {parts.map((part, i) =>
          part.type === 'text' ? (
            ...
            // Updated Citation Style: White background, Indigo border/text
            <button
              key={i}
              onClick={() => handleCitationClick(part)}
              className="inline-flex items-center bg-white border border-indigo-500 text-indigo-600 hover:bg-indigo-50 px-2 py-1 rounded text-xs font-bold transition-colors mx-1 cursor-pointer shadow-sm"
            >
  ...
  return (
    <div className="fixed inset-0 flex bg-white text-slate-900 overflow-hidden">
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
            ...
            {Object.entries(COUNTY_NAMES).map(([key, name]) => (
              <button
                ...
                className="w-full text-left p-4 border-2 border-slate-100 rounded-xl mb-2 hover:border-indigo-500 hover:bg-indigo-50 transition-all text-slate-700 font-medium"
              >
      ...
      {/* SIDEBAR - Slate 900 with Rainbow Line */}
      <div className={`fixed md:relative inset-y-0 left-0 w-80 bg-slate-900 text-white p-6 transition-transform duration-300 z-40 ${
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            {/* Line - Rainbow */}
            <div className="h-1 w-full bg-gradient-to-r from-amber-400 via-rose-400 to-indigo-400 rounded-full mt-1"></div>
          </div>
        ...
        {sidebarView === 'history' && (
          <div className="overflow-y-auto h-[calc(100vh-220px)] pr-2 custom-scrollbar">
            {/* New Chat - Rainbow Gradient */}
            <button
              className="w-full bg-gradient-to-r from-amber-500 via-rose-500 to-indigo-500 hover:opacity-90 text-white p-3 mb-4 rounded-xl font-bold text-sm shadow-sm transition-all flex items-center justify-center gap-2"
              ...
              <div
                className={`p-4 rounded-2xl max-w-[85%] lg:max-w-[75%] text-sm leading-relaxed shadow-sm ${
                  msg.role === 'assistant'
                    ...
                    : 'bg-gradient-to-br from-amber-500 via-rose-500 to-indigo-500 text-white' // User bubble is Rainbow
                }`}
              >
                {/* Only show avatar for Assistant */}
                {msg.role === 'assistant' && (
                  <div className="flex items-center gap-2 mb-2 border-b border-slate-100 pb-2">
                    <div className="w-5 h-5 rounded-full bg-gradient-to-br from-amber-400 via-rose-400 to-indigo-400 flex items-center justify-center text-[10px] text-white font-bold">
                      LM
                    </div>
        ...
        >
          <div className="max-w-4xl mx-auto relative flex items-center gap-3">
            ...
            <input
              ...
              className="flex-1 p-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-sm"
            />

            <button
              type="submit"
              disabled={isLoading}
              // Send button matches user bubble (Rainbow)
              className="px-6 py-3.5 bg-gradient-to-r from-amber-500 via-rose-500 to-indigo-500 hover:opacity-90 text-white font-bold rounded-xl disabled:opacity-50 disabled:cursor-not-allowed shadow-sm hover:shadow-md transition-all text-sm"
            >
              {isLoading ? '...' : 'Send'}
            </button>
          </div>
