...
export default function Dashboard() {
  ...
  // INITIAL MESSAGE - No "AI" mentions
  useEffect(() => {
      ...
      <>
        {parts.map((part, i) =>
          part.type === 'text' ? (
            ...
            // Updated Citation Style: White background, Slate border/text
            <button
              key={i}
              onClick={() => handleCitationClick(part)}
              className="inline-flex items-center bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 px-2 py-1 rounded text-xs font-bold transition-colors mx-1 cursor-pointer shadow-sm"
            >
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
            ...
            {Object.entries(COUNTY_NAMES).map(([key, name]) => (
              <button
                ...
                className="w-full text-left p-4 border-2 border-slate-100 rounded-xl mb-2 hover:border-[#059669] hover:bg-emerald-50 transition-all text-slate-700 font-medium"
              >
      ...
      <div className={`fixed md:relative inset-y-0 left-0 w-80 bg-slate-900 text-white p-6 transition-transform duration-300 z-40 ${
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            {/* Line - Emerald */}
            <div className="h-1 w-full bg-[#059669] rounded-full mt-1"></div>
          </div>
        ...
        {sidebarView === 'history' && (
          <div className="overflow-y-auto h-[calc(100vh-220px)] pr-2 custom-scrollbar">
            {/* New Chat - Emerald */}
            <button
              className="w-full bg-[#059669] hover:bg-[#047857] text-white p-3 mb-4 rounded-xl font-bold text-sm shadow-sm transition-all flex items-center justify-center gap-2"
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <button
                    ...
                    className="text-slate-500 hover:text-[#059669] opacity-0 group-hover:opacity-100 transition-all"
                  >
                    ✕
                  </button>
                </div>
              ...
              <div
                className={`p-4 rounded-2xl max-w-[85%] lg:max-w-[75%] text-sm leading-relaxed shadow-sm ${
                  msg.role === 'assistant'
                    ...
                    : 'bg-[#059669] text-white' // User bubble is Emerald
                }`}
              >
                {/* Only show avatar for Assistant */}
                {msg.role === 'assistant' && (
                  <div className="flex items-center gap-2 mb-2 border-b border-slate-100 pb-2">
                    <div className="w-5 h-5 rounded-full bg-[#059669] flex items-center justify-center text-[10px] text-white font-bold">
                      LM
                    </div>
        ...
        >
          <div className="max-w-4xl mx-auto relative flex items-center gap-3">
            ...
            <input
              ...
              className="flex-1 p-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#059669]/20 focus:border-[#059669] transition-all text-sm"
            />

            <button
              type="submit"
              disabled={isLoading}
              // Send button matches user bubble (Emerald)
              className="px-6 py-3.5 bg-[#059669] hover:bg-[#047857] text-white font-bold rounded-xl disabled:opacity-50 disabled:cursor-not-allowed shadow-sm hover:shadow-md transition-all text-sm"
            >
              {isLoading ? '...' : 'Send'}
            </button>
          </div>
