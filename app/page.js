...
const Icons = {
  ...
  FileText: () => <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10 9 9 9 8 9" /></svg>,
  ...
  <div className="w-full bg-white relative z-10 min-h-full flex flex-col">
    ...
    <section className="py-20 px-6 bg-white flex-1">
      <div className="max-w-6xl mx-auto">
        <h2 className={`text-xl font-semibold text-slate-900 mb-14 text-center tracking-tight ${outfit.className}`}>
          How it works
        </h2>
      ...
      <div className="bg-white border border-slate-200 rounded-xl w-full max-w-md p-10 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        {message && (
          <div className={`mt-6 p-4 rounded-lg text-sm border ${
            message.includes('Error') ? 'bg-red-50 border-red-200 text-red-900' : 'bg-slate-50 border-slate-200 text-slate-800'
          }`}>
            {message}
          </div>
        )}
...
const PricingModal = ({ isOpen, onClose, onCheckout, loading }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-white/95 flex items-center justify-center p-4">
      <div className="relative w-full max-w-3xl bg-white border border-slate-200 rounded-2xl p-8 md:p-10 shadow-2xl" onClick={e => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-6 right-6 text-slate-400 hover:text-slate-900 transition-colors">
          <Icons.X />
        </button>

        <div className="mb-8 text-center">
          <h3 className={`text-xs font-semibold text-slate-900 uppercase tracking-[0.25em] mb-3 ${outfit.className}`}>
            protocolLM
          </h3>
          <p className={`text-lg md:text-xl font-semibold text-slate-900 mb-2 tracking-tight ${outfit.className}`}>
            Choose your plan
          </p>
          <p className={`text-sm text-slate-600 max-w-xl mx-auto ${inter.className}`}>
            All plans include full access to Michigan Food Code, Washtenaw guidance, and image-based mock inspections.
          </p>
        </div>
        ...
        <div className="grid gap-4 md:gap-6 md:grid-cols-2">
            ...
            <div className="mt-6 space-y-2">
              <button
                ...
                className={`w-full bg-black hover:bg-slate-900 text-white font-semibold py-3.5 rounded-lg text-xs uppercase tracking-[0.18em] transition-colors ${
                  loading && loading !== 'business-monthly' ? 'opacity-60 cursor-not-allowed' : ''
                }`}
              >
                {loading === 'business-monthly' ? 'Processing...' : 'Start Trial'}
              </button>
              <button
                ...
                className={`w-full bg-white border border-dashed border-slate-400 text-slate-900 font-semibold py-3.5 rounded-lg text-xs uppercase tracking-[0.18em] hover:bg-slate-50 transition-colors ${
                  loading && loading !== 'business-annual' ? 'opacity-60 cursor-not-allowed' : ''
                }`}
              >
          ...
          <div className="border border-slate-900 rounded-xl p-6 flex flex-col justify-between bg-slate-950 text-slate-50 relative overflow-hidden">
            ...
            <div className="relative mt-6 space-y-2">
              <button
                onClick={() => onCheckout(ENTERPRISE_MONTHLY, 'enterprise-monthly')}
                ...
                className={`w-full bg-white text-black hover:bg-slate-100 font-semibold py-3.5 rounded-lg text-xs uppercase tracking-[0.18em] transition-colors ${
                  loading && loading !== 'enterprise-monthly' ? 'opacity-60 cursor-not-allowed' : ''
                }`}
              >
                {loading === 'enterprise-monthly' ? 'Processing...' : 'Start Trial'}
              </button>
              <button
                onClick={() => onCheckout(ENTERPRISE_ANNUAL, 'enterprise-annual')}
                ...
                className={`w-full bg-transparent border border-dashed border-slate-400 text-slate-100 font-semibold py-3.5 rounded-lg text-xs uppercase tracking-[0.18em] hover:bg-slate-900/60 transition-colors ${
                  loading && loading !== 'enterprise-annual' ? 'opacity-60 cursor-not-allowed' : ''
                }`}
              >
    ...
    const init = async () => {
      try {
        ...
        if (s) {
          ...
          // If no active sub, show pricing
          if (!active) setShowPricingModal(true)
        }
      } catch (e) {
  ...
  const handleSend = async (e) => {
    ...
    if (session && !activeChatId) {
      const { data: newChat } = await supabase
        .from('chats')
        .insert({ user_id: session.user.id, title: currentInput.slice(0, 30) + '...' })
        .select()
        .single()
      if (newChat) {
                  ...
                  {messages.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center p-6 text-center">
                      <p className={`text-slate-500 text-base max-w-md leading-relaxed ${inter.className}`}>
                        {activeMode === 'image'
                          ? 'Upload a photo to check for Priority items before your next inspection.'
                          : 'Ask about the Michigan Food Code, Washtenaw enforcement, or specific violations.'}
                      </p>
                    </div>
                  ) : (
                    <div className="flex flex-col w-full max-w-4xl mx-auto py-8 px-6 gap-8">
                      {messages.map((msg, idx) => (
                        ...
                        >
                          <div className={`max-w-[90%] px-2 ${
                            msg.role === 'user' ? 'text-slate-900 font-medium' : 'text-slate-800'
                          }`}>
                            {msg.image && (
                              <img
                                src={msg.image}
