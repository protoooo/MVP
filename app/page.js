...
const Icons = {
  ...
  Check: ({ color = 'text-[#EDEDED]' }) => <svg className={`w-4 h-4 ${color} shrink-0`} fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"/></svg>,
}
...
const InputBox = ({ input, setInput, handleSend, handleImage, isSending, fileInputRef, selectedImage, setSelectedImage, inputRef, activeMode, setActiveMode, session }) => {
  ...
  const getActiveColor = () => {
    switch (activeMode) {
      case 'chat': return '#3E7BFA'; case 'image': return '#F5A623'; case 'audit': return '#3ECF8E'; case 'critical': return '#EF4444';
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-2 md:px-4 pb-6 md:pb-0 z-20 relative">
      <div className="flex flex-col items-center w-full mb-3 md:mb-4">
        {/* Centered Modes for Mobile & Desktop */}
        <div className="flex items-center gap-1.5 px-2 md:px-0 overflow-x-auto no-scrollbar pb-1 scroll-smooth w-full justify-center">
          <button onClick={() => handleModeClick('chat')} className={`relative group flex items-center gap-1.5 px-2.5 md:px-3 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all duration-300 shrink-0 ${activeMode === 'chat' ? 'text-[#3E7BFA] bg-[#3E7BFA]/10' : 'text-[#A3A3A3] hover:text-[#EDEDED] hover:bg-[#1C1C1C]'}`}><Icons.MessageSquare /> <span>Chat</span></button>
          <button onClick={() => handleModeClick('image')} className={`relative group flex items-center gap-1.5 px-2.5 md:px-3 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all duration-300 shrink-0 ${activeMode === 'image' || selectedImage ? 'text-[#F5A623] bg-[#F5A623]/10' : 'text-[#A3A3A3] hover:text-[#EDEDED] hover:bg-[#1C1C1C]'}`}><Icons.Camera /> <span>Image</span></button>
          <button onClick={() => handleModeClick('audit')} className={`relative group flex items-center gap-1.5 px-2.5 md:px-3 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all duration-300 shrink-0 ${activeMode === 'audit' ? 'text-[#3ECF8E] bg-[#3ECF8E]/10' : 'text-[#A3A3A3] hover:text-[#EDEDED] hover:bg-[#1C1C1C]'}`}><Icons.ClipboardCheck /> <span>Audit</span></button>
          <button onClick={() => handleModeClick('critical')} className={`relative group flex items-center gap-1.5 px-2.5 md:px-3 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all duration-300 shrink-0 ${activeMode === 'critical' ? 'text-[#EF4444] bg-[#EF4444]/10' : 'text-[#A3A3A3] hover:text-[#EDEDED] hover:bg-[#1C1C1C]'}`}><Icons.Alert /> <span>Urgent</span></button>
        </div>
      </div>

      {selectedImage && (
        <div className="mb-2 mx-1 p-2 bg-[#1C1C1C] rounded-2xl inline-flex items-center gap-2 border border-[#F5A623]/30">
          <span className="text-xs text-[#F5A623] font-medium flex items-center gap-1"><Icons.Camera /> Analyzing Image</span>
          <button onClick={() => { setSelectedImage(null); setActiveMode('chat') }} className="text-[#525252] hover:text-white"><Icons.X /></button>
        </div>
      )}

      <form onSubmit={handleSend} className="relative flex items-end w-full bg-[#161616] border-2 rounded-[26px] shadow-sm transition-all duration-300 focus-within:ring-0 focus-within:outline-none" style={{ borderColor: `${activeColor}4D`, '--active-color': activeColor }}>
        <style jsx>{`form:focus-within { border-color: var(--active-color) !important; }`}</style>
        <input type="file" ref={fileInputRef} onChange={handleImage} accept="image/*" className="hidden" />
        <textarea ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(e) } }} placeholder={activeMode === 'chat' ? 'Ask anything...' : activeMode === 'image' ? 'Upload an image...' : activeMode === 'audit' ? 'Describe area to audit...' : 'Describe the emergency...'} className="flex-1 max-h=[200px] min-h-[50px] py-[13px] px-3 md:px-4 bg-transparent border-none focus:ring-0 outline-none focus:outline-none resize-none text-white placeholder-[#525252] text-sm md:text-[15px] leading-6" rows={1} style={{ height: 'auto', overflowY: 'hidden' }} />
        <button type="submit" disabled={(!input.trim() && !selectedImage) || isSending} className="p-2 md:p-2.5 m-1.5 rounded-full border transition-all flex items-center justify-center" style={{ backgroundColor: !input.trim() && !selectedImage ? '#2E2E2E' : activeColor, borderColor: !input.trim() && !selectedImage ? '#2E2E2E' : activeColor, color: !input.trim() && !selectedImage ? '#525252' : activeMode === 'chat' || activeMode === 'critical' || activeMode === 'audit' ? 'white' : 'black', cursor: !input.trim() && !selectedImage ? 'not-allowed' : 'pointer' }}>
          {isSending ? <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <Icons.Send />}
        </button>
      </form>
    </div>
  )
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-black flex items-center justify-center p-4 animate-in fade-in duration-500">
      <div className="relative w-full max-w-md bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-[0_0_60px_-15px_rgba(0,0,0,0.7)] animate-pop-in flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <h3 className="text-xs font-bold text-[#3E7BFA] uppercase tracking-[0.2em] mb-4 mt-2 text-center">protocolLM</h3>
        
        <div className="flex justify-center mb-8">
          <div className="bg-black/40 p-1 rounded-full flex relative border border-white/10">
            <button onClick={() => setBillingInterval('month')} className={`px-6 py-2 rounded-full text-xs font-bold transition-all duration-300 ${billingInterval === 'month' ? 'bg-[#3E7BFA] text-white shadow-lg' : 'text-white/50 hover:text-white'}`}>Monthly</button>
            <button onClick={() => setBillingInterval('year')} className={`px-6 py-2 rounded-full text-xs font-bold transition-all duration-300 flex items-center gap-2 ${billingInterval === 'year' ? 'bg-[#3E7BFA] text-white shadow-lg' : 'text-white/50 hover:text-white'}`}>Annual <span className="bg-[#22C55E] text-black text-[9px] px-1.5 py-0.5 rounded font-extrabold tracking-wide">SAVE $400</span></button>
          </div>
        </div>
        ...
        <ul className="space-y-4 mb-8 flex-1 border-t border-white/10 pt-6">
          <li className="flex items-start gap-3 text-sm font-medium text-white/90"><Icons.Check color="text-[#3E7BFA]" /> Unlimited Compliance Queries</li>
          <li className="flex items-start gap-3 text-sm font-medium text-white/90"><Icons.Check color="text-[#3E7BFA]" /> AI Visual Inspections (Image Mode)</li>
          <li className="flex items-start gap-3 text-sm font-medium text-white/90"><Icons.Check color="text-[#3E7BFA]" /> Full Washtenaw & FDA Database</li>
          <li className="flex items-start gap-3 text-sm font-medium text-white/90"><Icons.Check color="text-[#3E7BFA]" /> Mock Audit & Emergency Protocols</li>
          <li className="flex items-start gap-3 text-sm font-medium text-white/90"><Icons.Check color="text-[#3E7BFA]" /> <span className="text-[#3E7BFA]">Location License</span> (Unlimited Users)</li>
        </ul>

        <button onClick={() => handleCheckout(billingInterval === 'month' ? 'price_1SZi73DlSrKA3nbAzpQSbn5F' : 'price_1SZi9UDlSrKA3nbANUVnhH2D', 'protocollm')} disabled={loading !== null} className="w-full bg-[#3E7BFA] hover:bg-[#3469d4] text-white font-bold py-4 rounded-full text-sm uppercase tracking-[0.15em] transition-all shadow-[0_0_30px_-10px_rgba(62,123,250,0.6)] hover:shadow-[0_0_40px_-5px_rgba(62,123,250,0.8)] disabled:opacity-50 disabled:cursor-not-allowed border border-white/10">{loading === 'protocollm' ? 'Processing...' : 'Start 7-Day Free Trial'}</button>
      </div>
    </div>
  )
}
...
const AuthModal = ({ isOpen, onClose, message }) => {
  ...
  return (
    <div className="fixed inset-0 z-[999] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-[#181818] border border-[#2E2E2E] rounded-2xl w-full max-w-md p-8 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-start mb-6"><div><h2 className="text-xl font-semibold text-white mb-1">{message || 'Welcome to protocolLM'}</h2><p className="text-sm text-[#888888]">Sign in to continue your session</p></div><button onClick={onClose} className="text-[#888888] hover:text-white transition-colors"><Icons.X /></button></div>
        <button onClick={handleGoogleAuth} disabled={googleLoading || loading} className="w-full bg-white hover:bg-gray-200 text-black font-medium py-2.5 px-4 rounded-xl transition-colors flex items-center justify-center gap-3 mb-4">{googleLoading ? <div className="w-5 h-5 border-2 border-gray-400 border-t-black rounded-full animate-spin" /> : <svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844c-.209 1.125-.843 2.078-1.796 2.717v2.258h2.908c1.702-1.567 2.684-3.874 2.684-6.615z" fill="#4285F4" /><path d="M9.003 18c2.43 0 4.467-.806 5.956-2.18L12.05 13.56c-.806.54-1.836.86-3.047.86-2.344 0-4.328-1.584-5.036-3.711H.96v2.332C2.44 15.983 5.485 18 9.003 18z" fill="#34A853" /><path d="M3.964 10.71c-.18-.54-.282-1.117-.282-1.71s.102-1.17.282-1.71V4.958H.957C.347 6.173 0 7.55 0 9s.348 2.827.957 4.042l3.007-2.332z" fill="#FBBC05" /><path d="M9.003 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.464.891 11.426 0 9.003 0 5.485 0 2.44 2.017.96 4.958L3.967 7.29c.708-2.127 2.692-3.71 5.036-3.71z" fill="#EA4335" /></svg>} Continue with Google</button>
        <div className="relative my-6"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-[#2E2E2E]" /></div><div className="relative flex justify-center text-xs"><span className="bg-[#181818] px-3 text-[#888888]">OR</span></div></div>
        <form onSubmit={handleEmailAuth} className="space-y-4"><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email address" required className="w-full bg-[#0A0A0A] border border-[#2E2E2E] rounded-xl px-4 py-2.5 text-sm text-white placeholder-[#666666] focus:outline-none focus:border-[#3E7BFA] transition-all" /><button type="submit" disabled={loading || googleLoading} className="w-full bg-[#3E7BFA] hover:bg-[#3469d4] text-white font-medium py-2.5 rounded-full transition-colors">{loading ? 'Sending...' : 'Continue with Email'}</button></form>
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      } else { setProfile(null); setChatHistory([]); setHasActiveSubscription(false); setShowPricingModal(false) }
    })
  ...
  const handleSend = async (e) => {
    ...
    else if (activeMode === 'critical') finalInput = `[CRITICAL EMERGENCY MODE] The user is reporting a food safety emergency. Provide immediate, step-by-step corrective actions based on Imminent Health Hazard protocols. Be concise and authoritative. Input: ${input}`
  ...
  if (isLoading) return <div className="fixed inset-0 bg-[#121212] text-white flex items-center justify-center"><div className="w-6 h-6 border-2 border-[#3E7BFA] border-t-transparent rounded-full animate-spin" /></div>
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 w-full bg-[#121212] text-white overflow-hidden font-sans flex" style={{ height: '100dvh' }}>
        ...
        {session && (
          <aside className={`fixed inset-y-0 left-0 z-50 w-[260px] bg-[#121212] border-r border-[#2E2E2E] transform transition-transform duration-200 ease-in-out lg:relative lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
            <div className="p-3"><button onClick={handleNewChat} className="flex items-center justify-between w-full px-4 py-2 text-sm font-medium text-[#EDEDED] bg-transparent border border-[#2E2E2E] hover:bg-[#1C1C1C] rounded-full transition-colors group"><span className="flex items-center gap-2"><Icons.Plus /> New chat</span></button></div>
            <div className="flex-1 overflow-y-auto px-2"><div className="text-xs text-[#525252] font-medium px-2 py-4 uppercase tracking-wider">Recent</div><div className="space-y-1">{chatHistory.map((chat) => (<button key={chat.id} onClick={() => loadChat(chat.id)} className={`group w-full text-left px-3 py-2 text-sm rounded-2xl truncate transition-colors flex items-center justify-between ${currentChatId === chat.id ? 'bg-[#1C1C1C] text-white border border-[#333]' : 'text-[#888] hover:text-[#EDEDED] hover:bg-[#111] border border-transparent'}`}><div className="flex items-center gap-2 overflow-hidden"><Icons.ChatBubble /><span className="truncate">{chat.title || 'New Chat'}</span></div><div onClick={(e) => deleteChat(e, chat.id)} className="opacity-0 group-hover:opacity-100 p-1 text-[#525252] hover:text-red-400 transition-all"><Icons.Trash /></div></button>))}</div></div>
            {session && (<div className="p-3 border-t border-[#1C1C1C]"><div className="relative" ref={userMenuRef}><button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center gap-3 w-full px-3 py-2 hover:bg-[#1C1C1C] rounded-2xl transition-colors text-left border border-transparent hover:border-[#2E2E2E]"><div className="w-8 h-8 rounded-full bg-[#3E7BFA] flex items-center justify-center text-xs font-bold text-white shadow-lg shadow-blue-900/20">{session.user.email[0].toUpperCase()}</div><div className="flex-1 min-w-0"><div className="text-sm font-medium text-white truncate">{session.user.email}</div></div></button>{showUserMenu && (<div className="absolute bottom-full left-0 w-full mb-2 bg-[#1C1C1C] border border-[#2E2E2E] rounded-xl shadow-2xl overflow-hidden z-50 animate-in slide-in-from-bottom-2 fade-in duration-200"><button onClick={() => setShowPricingModal(true)} className="w-full px-4 py-3 text-left text-sm text-[#A1A1AA] hover:text-white hover:bg-[#262626] flex items-center gap-2"><Icons.Settings /> Subscription</button><div className="h-px bg-[#2E2E2E] mx-0" /><button onClick={(e) => handleSignOut(e)} className="w-full px-4 py-3 text-left text-sm text-red-400 hover:bg-[#262626] flex items-center gap-2"><Icons.SignOut /> Log out</button></div>)}</div></div>)}
          </aside>
        )}
          ...
          {!session ? (
            ...
            <>
              <div className="flex-1 overflow-y-auto" ref={scrollRef}>
                {messages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center p-4 text-center"><h1 className="text-2xl font-semibold text-white mb-2">What can I help with?</h1></div>
                ) : (
                    ...
                    {messages.map((msg, idx) => (
                      <div key={idx} className={`w-full flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[85%] ${msg.role === 'user' ? 'text-white px-2' : 'text-[#EDEDED] px-2'}`}>
                          {msg.image && <img src={msg.image} alt="Upload" className="rounded-2xl mb-3 max-h-60 object-contain border border-white/10" />}
