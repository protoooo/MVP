...
// ==========================================
// STYLES (MATTE LIQUID THEME)
// ==========================================
const GlobalStyles = () => (
  <style jsx global>{`
    body {
      background-color: #050507; /* Deep Void Black */
    ...
    /* MATTE GLASS CARD - The "Frosted Obsidian" Look */
    .matte-glass {
      background: rgba(20, 20, 25, 0.65); /* Semi-transparent dark base */
      backdrop-filter: blur(50px) saturate(120%); /* Heavy blur for matte effect */
      -webkit-backdrop-filter: blur(50px) saturate(120%);
      border: 1px solid rgba(255, 255, 255, 0.08); /* Ultra-thin border */
      box-shadow: 0 20px 40px -10px rgba(0, 0, 0, 0.5); /* Deep shadow */
    }
    
    /* INTERACTION */
    .squishy-press { transition: transform 0.2s cubic-bezier(0.34, 1.56, 0.64, 1); }
    .squishy-press:active { transform: scale(0.94); }
...
// ==========================================
// FLUID BACKGROUND (Blended Aurora)
// ==========================================
const FluidBackground = () => (
  <div className="fixed inset-0 z-0 overflow-hidden bg-[#020408]">
    {/* Emerald Glow - Soft & Large */}
    <div className="absolute top-[-20%] left-[-20%] w-[90vw] h-[90vw] bg-emerald-900/40 rounded-full mix-blend-screen filter blur-[150px] opacity-40 animate-aurora"></div>
    
    {/* Blue Glow - Soft & Large */}
    <div className="absolute bottom-[-20%] right-[-20%] w-[90vw] h-[90vw] bg-blue-900/40 rounded-full mix-blend-screen filter blur-[150px] opacity-40 animate-aurora delay-2000"></div>
    ...
    <>
      ...
      <div className="relative min-h-screen w-full overflow-hidden font-sans selection:bg-white/30">
        ...
        {/* =====================================
            ...
            ===================================== */}
        {!session && <div className="fixed inset-0 z-0 overflow-hidden bg-[#020408]">
            <div className="absolute top-[-10%] left-[-10%] w-[70vw] h-[70vw] bg-emerald-900/40 rounded-full mix-blend-screen filter blur-[150px] opacity-40 animate-blob"></div>
            <div className="absolute bottom-[-10%] right-[-10%] w-[70vw] h-[70vw] bg-blue-900/40 rounded-full mix-blend-screen filter blur-[150px] opacity-40 animate-aurora delay-2000"></div>
            <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E")`}}></div>
        </div>}
          ...
          <main className="flex-1 flex flex-col items-center justify-center px-4 w-full pb-20 md:pb-0 overflow-y-auto">
            ...
            {!session ? (
               <div className="w-full h-full flex flex-col items-center justify-center pb-[10vh]">
                  ...
                  <div className="text-center px-4 max-w-[600px] mb-12 animate-in fade-in zoom-in duration-1000">
                  ...
                  <div className="w-full max-w-5xl px-4 grid grid-cols-1 md:grid-cols-2 gap-8 z-20">
                     ...
                     <button onClick={() => triggerMode('image')} className="group relative glass-panel rounded-[32px] h-[420px] w-full hover:scale-[1.02] transition-all duration-500 text-left flex flex-col overflow-hidden hover:border-emerald-500/30 hover:shadow-[0_0_40px_-10px_rgba(16,185,129,0.3)]">
                     ...
                     <button onClick={() => triggerMode('chat')} className="group relative glass-panel rounded-[32px] h-[420px] w-full hover:scale-[1.02] transition-all duration-500 text-left flex flex-col overflow-hidden hover:border-blue-500/30 hover:shadow-[0_0_40px_-10px_rgba(59,130,246,0.3)]">
