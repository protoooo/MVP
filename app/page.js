...
const Icons = {
  ...
  Trash: () => <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>,
  ...
  Alert: () => <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>,
...
// ==========================================
// SOURCE TICKER COMPONENT (Single Instance)
// ==========================================
const SourceTicker = () => {
    ...
    <div className="flex justify-center mt-3">
      <div className="flex items-center justify-center px-4 py-2 rounded-full border border-white/10 bg-black/40 backdrop-blur-md shadow-lg">
        ...
        <div className="w-[260px] md:w-[310px] text-center overflow-hidden h-5 relative">
          <div
            key={index}
            className="absolute inset-0 flex items-center justify-start text-xs md:text-sm text-gray-200 font-mono tracking-wide animate-source-ticker uppercase truncate"
          >
...
// ==========================================
// INPUT COMPONENT (No Duplicate Ticker)
// ==========================================
const InputBox = ({
  input,
  setInput,
  ...
  const getActiveColor = () => {
    switch (activeMode) {
      case 'chat': return '#3E7BFA'
      case 'image': return '#F5A623'
      case 'audit': return '#3ECF8E'
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-2 md:px-4 pb-6 md:pb-0 z-20 relative">
      <div className="flex flex-col items-center w-full mb-3 md:mb-4">
        ...
        {/* Mobile: Scrollable with padding to prevent cutoff */}
        {/* Desktop: Centered */}
        <div className="flex items-center gap-1.5 px-2 md:px-0 overflow-x-auto no-scrollbar pb-1 scroll-smooth w-full md:justify-center justify-start">
          {/* Chat */}
          <button onClick={() => handleModeClick('chat')} className={`relative group flex items-center gap-1.5 px-2.5 md:px-3 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all duration-300 shrink-0 ${activeMode === 'chat' ? 'text-[#3E7BFA] bg-[#3E7BFA]/10' : 'text-[#A3A3A3] hover:text-[#EDEDED] hover:bg-[#1C1C1C]'}`}>
            <Icons.MessageSquare /> <span>Chat</span>
          </button>
          {/* Image */}
          <button onClick={() => handleModeClick('image')} className={`relative group flex items-center gap-1.5 px-2.5 md:px-3 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all duration-300 shrink-0 ${activeMode === 'image' || selectedImage ? 'text-[#F5A623] bg-[#F5A623]/10' : 'text-[#A3A3A3] hover:text-[#EDEDED] hover:bg-[#1C1C1C]'}`}>
            <Icons.Camera /> <span>Image</span>
          </button>
          {/* Audit */}
          <button onClick={() => handleModeClick('audit')} className={`relative group flex items-center gap-1.5 px-2.5 md:px-3 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all duration-300 shrink-0 ${activeMode === 'audit' ? 'text-[#3ECF8E] bg-[#3ECF8E]/10' : 'text-[#A3A3A3] hover:text-[#EDEDED] hover:bg-[#1C1C1C]'}`}>
            <Icons.ClipboardCheck /> <span>Audit</span>
          </button>
          {/* Urgent */}
          <button onClick={() => handleModeClick('critical')} className={`relative group flex items-center gap-1.5 px-2.5 md:px-3 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all duration-300 shrink-0 ${activeMode === 'critical' ? 'text-[#EF4444] bg-[#EF4444]/10' : 'text-[#A3A3A3] hover:text-[#EDEDED] hover:bg-[#1C1C1C]'}`}>
      ...
      <form onSubmit={handleSend} className="relative flex items-end w-full bg-[#161616] border-2 rounded-[26px] shadow-sm transition-all duration-300 focus-within:ring-0 focus-within:outline-none" style={{ borderColor: `${activeColor}4D`, '--active-color': activeColor }}>
        ...
        <textarea ref={inputRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(e) } }}
          placeholder={activeMode === 'chat' ? 'Ask anything...' : activeMode === 'image' ? 'Upload an image...' : activeMode === 'audit' ? 'Describe area to audit...' : 'Describe the emergency...'}
          className="flex-1 max-h=[200px] min-h-[50px] py-[13px] px-3 md:px-4 bg-transparent border-none focus:ring-0 outline-none focus:outline-none resize-none text-white placeholder-[#525252] text-sm md:text-[15px] leading-6" rows={1} style={{ height: 'auto', overflowY: 'hidden' }}
        />
...
const AuthModal = ({ isOpen, onClose, message }) => {
  ...
  const handleEmailAuth = async (e) => {
    ...
    if (error) setStatusMessage('Error: ' + error.message); else setStatusMessage('✓ Check your email for the login link.')
    setLoading(false)
  }
  ...
  const handleGoogleAuth = async () => {
    ...
    setStatusMessage('')
    const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: getRedirectUrl(), queryParams: { access_type: 'offline', prompt: 'consent' } } })
    if (error) { setStatusMessage('Error: ' + error.message); setGoogleLoading(false) }
  }
    ...
    <div className="fixed inset-0 z-[999] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-[#181818] border border-[#2E2E2E] rounded-2xl w-full max-w-md p-8 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleEmailAuth} className="space-y-4">
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email address" required className="w-full bg-[#0A0A0A] border border-[#2E2E2E] rounded-xl px-4 py-2.5 text-sm text-white placeholder-[#666666] focus:outline-none focus:border-[#3E7BFA] transition-all" />
          <button type="submit" disabled={loading || googleLoading} className="w-full bg-[#3E7BFA] hover:bg-[#3469d4] text-white font-medium py-2.5 rounded-full transition-colors">{loading ? 'Sending...' : 'Continue with Email'}</button>
        </form>
...
// ==========================================
// FULL SCREEN PRICING ("Glass Command Center")
// ==========================================
      ...
      >
          ...
          <div className="bg-black/40 p-1 rounded-full flex relative border border-white/10">
            <button
              ...
              className={`px-6 py-2 rounded-full text-xs font-bold transition-all duration-300 ${
                billingInterval === 'month' ? 'bg-[#3E7BFA] text-white shadow-lg' : 'text-white/50 hover:text-white'
              }`}
            >
              Monthly
            </button>
            <button
              ...
              className={`px-6 py-2 rounded-full text-xs font-bold transition-all duration-300 flex items-center gap-2 ${
                billingInterval === 'year' ? 'bg-[#3E7BFA] text-white shadow-lg' : 'text-white/50 hover:text-white'
              }`}
            >
        ...
        <button
          onClick={() => handleCheckout(
            billingInterval === 'month' ? 'price_1SZi73DlSrKA3nbAzpQSbn5F' : 'price_1SZi9UDlSrKA3nbANUVnhH2D', 
            'protocollm'
          )}
          disabled={loading !== null}
          className="w-full bg-[#3E7BFA] hover:bg-[#3469d4] text-white font-bold py-4 rounded-full text-sm uppercase tracking-[0.15em] transition-all shadow-[0_0_30px_-10px_rgba(62,123,250,0.6)] hover:shadow-[0_0_40px_-5px_rgba(62,123,250,0.8)] disabled:opacity-50 disabled:cursor-not-allowed border border-white/10"
        >
