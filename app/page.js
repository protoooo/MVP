...
const DemoChatInterface = () => {
  ...
  return (
    // Mobile: 350px height / Desktop: 550px height
    <div className="w-full max-w-3xl mx-auto bg-white border-2 border-slate-200/80 rounded-xl shadow-2xl shadow-slate-200/50 overflow-hidden flex flex-col h-[350px] md:h-[550px] relative">
      ...
      {/* MESSAGES AREA */}
      <div ref={scrollRef} className="flex-1 p-4 md:p-6 overflow-y-auto space-y-4 md:space-y-6 bg-white scroll-smooth">
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-500`}>
            <div className={`max-w-[85%] px-4 py-3 md:px-5 md:py-3 rounded-xl text-xs md:text-sm leading-relaxed font-medium shadow-sm ${
              msg.role === 'user' 
      ...
      <div className="p-3 md:p-5 bg-white border-t border-slate-100">
        <div className="w-full bg-[#f8fafc] border border-slate-200 rounded-lg p-3 flex items-center gap-3 shadow-inner">
           ...
           <div className="flex-1 text-xs md:text-sm text-slate-700 font-mono font-medium h-[20px] overflow-hidden relative flex items-center">
              {inputValue}
              {isTyping && <span className="inline-block w-0.5 h-4 bg-[#6b85a3] ml-0.5 animate-pulse"></span>}
              {!inputValue && !isTyping && <span className="text-slate-300 text-[10px] md:text-xs uppercase tracking-wide">Enter regulatory query...</span>}
           </div>

           <div className={`w-7 h-7 rounded-md flex items-center justify-center transition-all duration-300 shrink-0 ${inputValue ? 'bg-[#6b85a3]' : 'bg-slate-200'}`}>
...
function MainContent() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#f8fafc] font-mono text-slate-900 selection:bg-[#6b85a3] selection:text-white flex flex-col">
      ...
      {/* HEADER */}
      <nav className="w-full max-w-7xl mx-auto px-6 py-6 flex justify-between items-center fixed top-0 left-0 right-0 z-20 bg-[#f8fafc]/90 backdrop-blur-sm border-b border-slate-200/50">
        <div className={`transition-all duration-1000 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tighter text-slate-900">
            protocol<span style={{ color: '#6b85a3' }}>LM</span>
          </h1>
        </div>
        
        {/* RESPONSIVE NAV: Smaller Gap on Mobile */}
        <div className={`flex gap-3 md:gap-6 text-[10px] md:text-xs font-bold uppercase tracking-widest transition-all duration-1000 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
          <button onClick={() => router.push('/pricing')} className="px-2 md:px-4 py-2 text-slate-500 hover:text-[#6b85a3] transition-colors">
            Pricing
          </button>
          <button onClick={() => openAuth('login')} className="px-2 md:px-4 py-2 text-slate-500 hover:text-[#6b85a3] transition-colors">
            Sign In
          </button>
          <button onClick={() => openAuth('signup')} className="px-3 md:px-5 py-2 md:py-2.5 text-[#6b85a3] border border-[#6b85a3] rounded-lg hover:bg-[#6b85a3] hover:text-white transition-all">
            Create Account
          </button>
        </div>
      </nav>
      ...
      <div className="flex-1 w-full max-w-5xl mx-auto px-4 flex flex-col items-center justify-center pt-24 md:pt-16">
        
        {/* HERO TEXT */}
        <div className={`text-center mb-8 md:mb-12 transition-all duration-1000 delay-100 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} w-full`}>
          
          <h2 className="text-2xl md:text-4xl font-mono font-medium text-slate-900 tracking-tight leading-tight mb-6 whitespace-normal lg:whitespace-nowrap">
      ...
      {/* FOOTER */}
      <div className="w-full py-8 md:py-12 text-center bg-white border-t border-slate-200 mt-8">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row justify-center items-center gap-4">
          <div className="flex gap-6 md:gap-8 text-[10px] font-bold uppercase tracking-widest text-slate-500">
