    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#f8fafc]/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-sm bg-white border border-slate-200 shadow-2xl p-8 rounded-xl relative">
        ...
        <div className="space-y-4">
          ...
          <button onClick={handleAuth} disabled={loading} className="w-full bg-[#6b85a3] hover:bg-[#5a728a] text-white font-bold py-3.5 rounded-lg text-xs uppercase tracking-widest transition-all font-mono shadow-md active:scale-95">{loading ? 'Processing...' : (view === 'signup' ? 'Create Account' : 'Sign In')}</button>
        </div>
...
function MainContent() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#f8fafc] font-sans text-slate-900 selection:bg-[#6b85a3] selection:text-white flex flex-col relative overflow-hidden">
      
      {/* --- BACKGROUND IMAGE with OVERLAY --- */}
      {/* 1. Add your 'background.png' to the 'public' folder */}
      {/* 2. This code places it behind the content, faded out so text pops */}
      <div className="absolute inset-0 z-0 pointer-events-none">
         <Image 
           src="/background.png" 
           alt="Background" 
           fill 
           className="object-cover opacity-[0.08] grayscale" // 8% Opacity + B&W for professional look
           priority
         />
      ...
      <nav className="w-full max-w-7xl mx-auto px-6 py-6 flex justify-between items-center fixed top-0 left-0 right-0 z-30 bg-[#f8fafc]/90 backdrop-blur-sm">
      ...
      <div className="flex-1 w-full max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-center pt-28 pb-24 gap-16 relative z-10">
        <div className={`flex-1 text-center md:text-left transition-all duration-1000 delay-100 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          {/* SIMPLE COMPACT DATA LABEL - Clean & Aligned */}
      ...
      <div className="w-full py-8 text-center border-t border-slate-200 relative z-10">
