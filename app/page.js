...
const DemoChatInterface = () => {
  ...
  useEffect(() => {
    ...
    const typeChar = async (char) => {
      ...
      // Randomize typing speed (30ms - 90ms)
    ...
    const runSimulation = async () => {
      while (isMounted) {
        ...
        for (const step of SEQUENCE) {
          ...
          // 4. AI Thinking
          setIsThinking(true)
          await wait(1500) // Processing time
          setIsThinking(false)
  ...
  return (
    <div className="w-full max-w-3xl mx-auto bg-white border-2 border-slate-200/80 rounded-xl shadow-2xl shadow-slate-200/50 overflow-hidden flex flex-col h-[500px] md:h-[600px] relative">
      
      {/* CHAT HEADER */}
      <div className="h-14 border-b border-slate-100 bg-[#f8fafc] flex items-center px-6 gap-3">
      ...
      <div ref={scrollRef} className="flex-1 p-6 overflow-y-auto space-y-6 bg-white scroll-smooth">
            ...
            <div className={`max-w-[85%] px-6 py-4 rounded-2xl text-sm leading-relaxed font-medium shadow-sm ${
              msg.role === 'user' 
               ...
               <div className="whitespace-pre-wrap font-mono">
                 {msg.role === 'assistant' && <span className="block text-[10px] font-bold text-slate-400 mb-2 uppercase tracking-wider">protocol_lm</span>}
                 {msg.content}
               </div>
            </div>
          </div>
        ...
        {isThinking && (
          <div className="flex justify-start animate-in fade-in duration-300">
             <div className="bg-[#f8fafc] px-5 py-4 rounded-2xl rounded-tl-sm border border-slate-100 flex gap-1.5 items-center h-[44px]">
      ...
      <div className="p-5 bg-white border-t border-slate-100">
        <div className="w-full bg-[#f8fafc] border border-slate-200 rounded-lg p-4 flex items-center gap-4 shadow-inner">
           ...
           <div className="flex-1 text-sm text-slate-700 font-mono font-medium h-[20px] overflow-hidden relative flex items-center">
              {inputValue}
              {/* Cursor Blinks only when typing */}
              {isTyping && <span className="inline-block w-0.5 h-4 bg-[#6b85a3] ml-0.5 animate-pulse"></span>}
              {!inputValue && !isTyping && <span className="text-slate-300">Enter regulatory query...</span>}
           </div>

           <div className={`w-8 h-8 rounded-md flex items-center justify-center transition-all duration-300 ${inputValue ? 'bg-[#6b85a3]' : 'bg-slate-200'}`}>
...
function MainContent() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#f8fafc] font-mono text-slate-900 selection:bg-[#6b85a3] selection:text-white flex flex-col">
      ...
      <div className={`text-center pt-8 pb-12 transition-all duration-1000 delay-100 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} w-full px-6`}>
      ...
      {/* CHAT DEMO (Bottom 2/3 Area) */}
      <div className={`flex-1 w-full max-w-5xl mx-auto px-4 flex flex-col justify-end pb-12 transition-all duration-1000 delay-200 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
        <DemoChatInterface />
      </div>
