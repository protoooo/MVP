...
const DemoChatInterface = () => {
  ...
  return (
    ...
    <div className="w-full max-w-3xl mx-auto bg-white border-2 border-slate-200/80 rounded-xl shadow-2xl shadow-slate-200/50 overflow-hidden flex flex-col h-[420px] relative">
      
      <div className="h-10 border-b border-slate-100 bg-[#f8fafc] flex items-center px-4 gap-2">
      ...
      <div ref={scrollRef} className="flex-1 p-4 overflow-y-auto space-y-4 bg-white scroll-smooth">
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-500`}>
            <div className={`max-w-[85%] px-4 py-3 rounded-xl text-xs md:text-sm leading-relaxed font-medium shadow-sm ${
              msg.role === 'user' 
                ? 'bg-[#6b85a3] text-white rounded-tr-sm' 
                : 'bg-[#f8fafc] text-slate-700 rounded-tl-sm border border-slate-100'
            }`}>
               <div className="whitespace-pre-wrap font-mono">
                 {msg.role === 'assistant' && <span className="block text-[9px] font-bold text-slate-400 mb-1 uppercase tracking-wider">protocol_lm</span>}
                 {msg.content}
               </div>
            </div>
          </div>
        ...
        {isThinking && (
          <div className="flex justify-start animate-in fade-in duration-300">
             <div className="bg-[#f8fafc] px-4 py-3 rounded-xl rounded-tl-sm border border-slate-100 flex gap-1.5 items-center h-[36px]">
...
function MainContent() {
  ...
  return (
    // FLEX COLUMN LAYOUT WITH "JUSTIFY-BETWEEN" TO PUSH CONTENT APART
    <div className="min-h-screen w-full bg-[#f8fafc] font-mono text-slate-900 selection:bg-[#6b85a3] selection:text-white flex flex-col justify-between">
      
      {/* HEADER */}
      <nav className="w-full max-w-7xl mx-auto px-6 py-6 flex justify-between items-center bg-[#f8fafc]/90 backdrop-blur-sm z-20">
      ...
      <div className="flex-1 w-full max-w-5xl mx-auto px-6 flex flex-col items-center justify-center -mt-12">
      ...
      {/* FOOTER */}
      <div className="w-full py-8 text-center bg-[#f8fafc] border-t border-slate-200">
