// lib/captchaVerification.js - SECURITY FIX: Strict CAPTCHA enforcement in production
...
export async function verifyCaptcha(token, action = 'submit', remoteIp = null) {
  // ✅ SECURITY FIX: In production, ALWAYS require CAPTCHA
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // ✅ SECURITY: In production, fail closed if Cloudflare API is down
      if (process.env.NODE_ENV === 'production') {
    ...
    // ✅ SECURITY: In production, fail closed on exceptions
    if (process.env.NODE_ENV === 'production') {
      return { success: false, error: 'CAPTCHA verification unavailable' }
    }
