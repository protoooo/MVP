...
async function loadConversationSchema(supabase) {
  ...
  try {
    ...
    if (error) {
      logger.warn('Conversation memory schema lookup failed', { error: error.message })
      conversationSchema = { exists: false }
      return conversationSchema
    }

    const columns = new Set((data || []).map((c) => c.column_name))
    conversationSchema = {
      exists: columns.size > 0,
      hasMetadata: columns.has('metadata'),
      hasKeyTopics: columns.has('key_topics'),
      hasLastContext: columns.has('last_context'),
    }

    if (!conversationSchema.exists) {
      logger.warn('conversation_memory table missing - skipping memory features')
    } else if (!conversationSchema.hasMetadata) {
      logger.warn('conversation_memory missing metadata column - metadata will be omitted')
    }
  } catch (err) {
    logger.warn('Conversation memory schema lookup exception', { error: err?.message })
    conversationSchema = { exists: false }
  }
