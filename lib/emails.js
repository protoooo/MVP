...
export const emails = {
  ...
  trialEnded: async (userEmail, userName) => {
    ...
    const content = `
      ...
      <p style="color: #334155; font-size: 16px; line-height: 1.6; margin: 0 0 24px 0;">
        To continue using protocolLM, subscribe for just $100/month—less than the cost of a single Priority violation fine.
      </p>
      ...
      <p style="color: #334155; font-size: 16px; line-height: 1.6; margin: 0 0 16px 0;">
        One avoided violation fine ($200-500) pays for 2-5 months of protocolLM.
      </p>
  ...
  locationManagerInvite: async (managerEmail, restaurantName, signupUrl, buyerEmail) => {
    ...
    <p style="color: #334155; font-size: 16px; line-height: 1.6; margin: 0 0 24px 0;">
      protocolLM helps catch health code violations before inspectors arrive - think of it as your 24/7 compliance assistant for Washtenaw County.
    </p>
  ...
  multiLocationPurchaseComplete: async (buyerEmail, buyerName, locationCount, signupLinks) => {
    ...
    const html = emailTemplate({
      ...
      content,
      footerNote: `💳 Billing: $${149 * count}/month for ${count} locations • Each location has its own account`,
    })
