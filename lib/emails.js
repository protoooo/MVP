...
export const emails = {
  ...
  trialStarted: async (userEmail, userName) => {
    ...
    const subject = 'Welcome to protocolLM - Your 14-day trial is active'
    ...
    const content = `
      ...
      <p style="color: #334155; font-size: 16px; line-height: 1.6; margin: 0 0 24px 0;">
        Your 14-day trial is now active! You have full access to protocolLM—no credit card required yet.
      </p>
    ...
    const html = emailTemplate({
      ...
      preheader: 'Your 14-day trial is active. Get started in 3 steps.',
      content,
      ctaText: 'Open protocolLM',
      ctaUrl: APP_URL,
      footerNote: '⏰ Your trial ends in 14 days. No credit card required during trial.',
    })
  ...
  multiLocationPurchaseComplete: async (buyerEmail, buyerName, locationCount, devicesPerLocation, totalMonthly, signupLinks) => {
    ...
    const computedTotal = Number.isFinite(Number(totalMonthly))
      ? Number(totalMonthly)
      : (count * PRICE_MONTHLY) + Math.max(0, count * devices - count) * (devices > 1 ? 15 : 0)
