...
export async function validateSingleLocation(userId, sessionInfo = {}) {
  try {
    ...
    const registeredFingerprint = metadata.device_fingerprint || metadata.location_hash
    const currentFingerprint = registeredFingerprint || generateDeviceFingerprint(sessionInfo)
    ...
    const rogueFingerprints = new Set()

    for (const record of recentDevices || []) {
      if (registeredFingerprint) {
        if (record.location_fingerprint === registeredFingerprint) {
          uniqueDevices.add(registeredFingerprint)
        } else {
          rogueFingerprints.add(record.location_fingerprint)
        }
      } else {
        uniqueDevices.add(record.location_fingerprint)
      }
    }
    ...
    if (registeredFingerprint && rogueFingerprints.size > 0) {
      logger.security('Additional device fingerprints detected - using registered fingerprint', {
        userId,
        registeredFingerprint: registeredFingerprint.substring(0, 8) + '***',
        rogueCount: rogueFingerprints.size,
      })
    }

    const effectiveDeviceCount = registeredFingerprint ? 1 : uniqueDevices.size

    if (!registeredFingerprint && uniqueDevices.size > MAX_DEVICES_PER_LICENSE) {
    ...
    return {
      valid: true,
      deviceFingerprint: currentFingerprint,
      uniqueDevicesUsed: effectiveDeviceCount,
    }
