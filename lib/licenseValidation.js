// lib/licenseValidation.js - MULTI-LOCATION FRIENDLY VERSION
...
// ✅ UPDATED: More lenient for multi-unit operators
const MAX_LOCATIONS_SINGLE = 2  // Allow 2 locations before requiring upgrade
const LOCATION_WINDOW_DAYS = 7
const GRACE_PERIOD_DAYS = 3  // 3-day grace period for upgrade
...
/**
 * ✅ IMPROVED: Gentle enforcement with upgrade prompts
 */
    ...
    if (uniqueLocations.size > MAX_LOCATIONS_SINGLE) {
      ...
      if (inGracePeriod) {
        ...
        return {
          ...
          message: `We detected ${uniqueLocations.size} locations in the past week. You have ${daysRemaining} day${daysRemaining > 1 ? 's' : ''} to upgrade to multi-location pricing ($${100 * uniqueLocations.size}/month for ${uniqueLocations.size} locations).`
        }
      }
      ...
      return {
        ...
        requiresUpgrade: true,
        uniqueLocationsUsed: uniqueLocations.size,
        suggestedPrice: 100 * uniqueLocations.size,
