// lib/licenseValidation.js - FIXED: More lenient multi-location thresholds
...
// ✅ UPDATED: More lenient for legitimate multi-unit operators
const MAX_LOCATIONS_SINGLE = 3  // Changed from 2 to 3
const LOCATION_WINDOW_DAYS = 7
const GRACE_PERIOD_DAYS = 5  // Changed from 3 to 5 days
...
export async function validateSingleLocation(userId, sessionInfo = {}) {
  try {
    ...
    // ✅ IMPROVED: Gradual enforcement with extended grace period
    if (uniqueLocations.size > MAX_LOCATIONS_SINGLE) {
      ...
      if (inGracePeriod) {
        ...
        return {
          ...
          message: `We detected ${uniqueLocations.size} locations in the past week. You have ${daysRemaining} day${daysRemaining > 1 ? 's' : ''} to upgrade to multi-location pricing ($${99 * uniqueLocations.size}/month for ${uniqueLocations.size} locations).`
        }
      }
      ...
      return {
        ...
        requiresUpgrade: true,
        uniqueLocationsUsed: uniqueLocations.size,
        suggestedPrice: 99 * uniqueLocations.size,
