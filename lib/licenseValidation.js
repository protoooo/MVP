...
async function getSeatInventory(userId) {
  const [{ data: purchaserSeats, error: purchaserError }, { data: claimedSeats, error: claimedError }] =
    await Promise.all([
      supabase.from('device_seats').select('*').eq('purchaser_user_id', userId),
      supabase.from('device_seats').select('*').eq('claimed_user_id', userId),
    ])

  if (purchaserError || claimedError) {
    logger.error('Failed to load seats', { userId, purchaserError: purchaserError?.message, claimedError: claimedError?.message })
    return []
  }

  const merged = [...(purchaserSeats || []), ...(claimedSeats || [])]
