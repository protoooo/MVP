...
export function validatePromptInputs(profile, documentChunks, metadata) {
  ...
  if (profile?.plain_language_rules && profile.plain_language_rules.length > 50) {
    warnings.push('Profile has many rules - may exceed token limits')
  }
  
  if (documentChunks && documentChunks.length > 10) {
    warnings.push('Many document chunks provided - may exceed token limits')
  }
