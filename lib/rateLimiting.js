...
// Each feature has daily AND weekly limits that must both be satisfied
export const RATE_LIMITS = {
  KNOWLEDGE_BASE_SEARCH_DAILY: {
    limit: 5,
    windowMinutes: 24 * 60, // 24 hours
    type: 'kb_search_daily'
  },
  KNOWLEDGE_BASE_SEARCH_WEEKLY: {
    limit: 20,
    windowMinutes: 7 * 24 * 60, // 7 days
    type: 'kb_search_weekly'
  },
  FREE_IMAGE_ANALYSIS_DAILY: {
    limit: 3,
    windowMinutes: 24 * 60, // 24 hours
    type: 'free_image_daily'
  },
  FREE_IMAGE_ANALYSIS_WEEKLY: {
    limit: 10,
    windowMinutes: 7 * 24 * 60, // 7 days
    type: 'free_image_weekly'
  }
}
...
/**
 * Check multiple rate limits (e.g., daily AND weekly)
 * @param {string} identifier - IP address
 * @param {object[]} configs - Array of rate limit configurations to check
 * @returns {Promise<{allowed: boolean, remaining: number, resetAt: Date, limitType?: string}>}
 */
export async function checkMultipleRateLimits(identifier, configs) {
  ...
  // Find the most restrictive limit (lowest remaining or first blocked)
  ...
  const mostRestrictive = results.find(r => r.remaining === minRemaining)
