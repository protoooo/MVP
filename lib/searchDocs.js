...
function initializeClients() {
  ...
  try {
    ...
    if (!vertexAI) {
      ...
      // Parse JSON credentials if on Railway
...
export async function searchDocuments(query, county = 'washtenaw', topK = 20) {
  ...
  try {
    ...
    // FIXED: Use the correct embedding model for Gemini 2.0
    const model = ai.preview.getGenerativeModel({ 
      model: "text-embedding-004"
    })
    
    // FIXED: Use embedContent method (correct for text-embedding-004)
    ...
    if (!queryEmbedding || !Array.isArray(queryEmbedding)) {
      console.error("[SearchDocs] Vertex AI returned invalid embedding format")
      return []
    }
