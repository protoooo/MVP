...
function initializeClients() {
  ...
  try {
    ...
    if (projectId && credentialsJson && !vertex_ai) {
      const tempFilePath = path.join('/tmp', 'google-credentials.json')
      // Ensure we don't overwrite if it exists and is valid, but for safety in serverless, write it
      try { fs.writeFileSync(tempFilePath, credentialsJson) } catch(e) {}
...
// ... (Helper functions match previous logic) ...
...
export async function searchDocuments(query, county = 'washtenaw', topK = 25) {
  ...
  try {
    ...
    // Vertex returns embeddings slightly differently
