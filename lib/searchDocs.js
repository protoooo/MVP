// lib/searchDocs.js - FIXED VERSION with better retrieval
...
const FETCH_MULTIPLIER = 3 // Increased for better results
...
export async function searchDocuments(query, county = 'washtenaw', topK = 20) {
  ...
  try {
    ...
    // ✅ CRITICAL: Generate embedding with error handling
    let queryEmbedding
    try {
    ...
    // ✅ CRITICAL: Use proper match_documents function call
    try {
      const { data: documents, error } = await Promise.race([
        supabase.rpc('match_documents', {
          ...
          filter_county: null, // No county filter since you only have Washtenaw
        }),
        new Promise((_, reject) =>
      ...
      // ✅ CRITICAL: Proper result formatting
      const results = documents.map(doc => ({
        source: doc.metadata?.source || doc.filename || 'Unknown Source',
        page: doc.metadata?.page_estimate || doc.metadata?.page || doc.chunk_index || 'N/A',
      ...
      if (finalResults.length > 0) {
        logger.debug('Sample search results', {
          sample: finalResults.slice(0, 3).map(r => ({
            source: r.source,
