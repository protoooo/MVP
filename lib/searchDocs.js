...
// ✅ UPDATED: Use configurable Cohere embed model with dimension validation
...
const searchCache = global.__searchDocsCache || (global.__searchDocsCache = new Map())
...
// ✅ Avoid stacking intervals in dev / hot reload / long-lived instances
if (!global.__searchDocsCleanupInterval) {
  global.__searchDocsCleanupInterval = setInterval(cleanupCache, 5 * 60 * 1000)
}
...
export async function searchDocuments(query, county = 'washtenaw', topK = 20) {
  ...
  try {
    ...
    const safeCounty = VALID_COUNTIES.includes(String(county).toLowerCase())
