// lib/searchDocs.js - Fixed county filter for Washtenaw-only deployment
...
export async function searchDocuments(query, county = 'washtenaw', topK = 20) {
  ...
  try {
    ...
    // ✅ FIXED: Pass null instead of county filter since you only have Washtenaw data
    // This prevents search failures if metadata doesn't have county field
    const searchPromise = supabase.rpc('match_documents', {
      query_embedding: queryEmbedding,
      match_threshold: 0.1,
      match_count: fetchCount,
      filter_county: null, // Changed from safeCounty - remove filter for Washtenaw-only deployment
    })
    ...
    const results = documents.map(doc => ({
      ...
      county: doc.metadata?.county || 'washtenaw' // Default to washtenaw
    }))
