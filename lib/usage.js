// lib/usage.js - FIXED: Properly sets both 'plan' and 'plan_type'
...
export async function logUsageForAnalytics(payload) {
  ...
  // ✅ FIXED: Extract plan name from subscription
  // Priority: 1) sub.plan, 2) parse price_id, 3) default to 'business'
  ...
  try {
    ...
    if (!inSamePeriod) {
      // ✅ FIXED: Set BOTH 'plan' (NOT NULL) and 'plan_type' (nullable)
      const { data: inserted, error: insertError } = await supabaseAdmin
        .from('usage_counters')
        .insert({
          user_id: userId,
          plan: planName,           // ✅ NOT NULL column
          plan_type: 'metered',     // ✅ Nullable column
