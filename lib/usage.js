...
/**
 * ✅ FIXED: Enhanced Stripe verification for trial expiration
 */
async function verifyTrialExpiration(sub, userId) {
  ...
  try {
    ...
    // If Stripe API fails, use minimal fallback grace period
...
/**
 * ✅ FIXED: Check access with enhanced trial-to-paid conversion
 */
export async function checkAccess(userId) {
  ...
  // ✅ FIXED: Enhanced trial expiration check with Stripe verification
...
/**
 * ✅ Log usage (unchanged - already has Stripe verification)
 */
export async function logUsageForAnalytics(payload) {
  ...
  // ✅ Already has Stripe verification for trial expiration
