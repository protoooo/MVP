// lib/usage.js - FIXED: Enhanced trial conversion with Stripe payment verification
...
/**
 * ✅ FIXED: Check access with enhanced trial-to-paid conversion + Stripe verification
 */
export async function checkAccess(userId) {
  ...
  // ========================================================================
  // ✅ FIXED: Enhanced trial expiration check with Stripe payment verification
  // ========================================================================
  if (sub.status === 'trialing' && sub.trial_end) {
    ...
    if (trialEnd < now) {
      // Trial has expired - check payment method AND verify with Stripe
      const hasPaymentMethod = sub.stripe_subscription_id && !sub.cancel_at_period_end
      
      if (hasPaymentMethod) {
        try {
          // ✅ NEW: Verify actual Stripe subscription status
          ...
          if (stripeSubscription.latest_invoice?.payment_intent?.status === 'processing') {
            // ✅ REDUCED: 15 minute grace period (down from 1 hour)
          ...
          // If Stripe API fails, log but don't block (fail open for Stripe outages)
          logger.error('Stripe verification failed during trial check', {
            error: stripeError.message,
            userId
          })
          
          // ✅ FALLBACK: Use minimal grace period if Stripe unreachable
          ...
          // Even fallback grace period expired
...
export async function logUsageForAnalytics(payload) {
  ...
  // ========================================================================
  // ✅ ENHANCED: Check trial expiration (with Stripe verification)
  // ========================================================================
  if (sub.status === 'trialing' && sub.trial_end) {
    const trialEnd = new Date(sub.trial_end)
    const now = new Date()
    ...
    if (trialEnd < now) {
      ...
      if (hasPaymentMethod) {
        try {
          ...
          if (stripeSubscription.status === 'active') {
            // Payment succeeded, allow usage
            logger.info('Usage logged - trial converted to active', { userId })
          } else if (stripeSubscription.latest_invoice?.payment_intent?.status === 'processing') {
            // Check grace period
            const GRACE_PERIOD_MINUTES = 15
            const gracePeriodEnd = new Date(trialEnd.getTime() + GRACE_PERIOD_MINUTES * 60 * 1000)
            
            if (now > gracePeriodEnd) {
              logger.warn('Trial expired during usage logging (past grace period)', { userId })
            ...
            // Payment failed
            logger.warn('Trial expired during usage - payment failed', { 
              userId,
              stripeStatus: stripeSubscription.status 
            })
            const e = new Error('Your trial has ended and payment failed. Please update your payment method.')
          ...
          // Stripe API failure - use minimal fallback
          logger.error('Stripe verification failed during usage logging', {
            error: stripeError.message,
            userId
          })
          ...
          logger.warn('Usage logged with fallback grace period', { userId })
        }
      } else {
  ...
  try {
    ...
    if (err?.code === 'NO_SUBSCRIPTION' || err?.code === 'TRIAL_EXPIRED' || err?.code === 'SUBSCRIPTION_EXPIRED' || err?.code === 'TRIAL_EXPIRED_PAYMENT_ISSUE' || err?.code === 'TRIAL_EXPIRED_PAYMENT_FAILED' || err?.code === 'TRIAL_EXPIRED_VERIFICATION_FAILED') {
      throw err
    }
