// lib/usage.js - STRICT trial enforcement with card-required flow
...
/**
 * Check if user has valid access (STRICT enforcement)
 */
export async function checkAccess(userId) {
  ...
  if (sub.status === 'active' && sub.current_period_end) {
    ...
    if (periodEnd < new Date()) {
      logger.warn('Subscription period ended', { userId, periodEnd: periodEnd.toISOString() })
