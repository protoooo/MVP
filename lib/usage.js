...
// Usage config: ~ $20/month API cost per heavy tenant
const USAGE_CONFIG = {
  monthlyUnitCap: 1300,   // 1,300 usage units per billing period
  textWeight: 1,          // 1 text request = 1 unit
  imageWeight: 2          // 1 image request = 2 units
}
...
export async function logUsageForAnalytics(userId, { isImage = false } = {}) {
  ...
  try {
    ...
    if (!inSamePeriod) {
      ...
      if (insertError) {
        logger.error('Failed to create usage row', { error: insertError.message, userId })
      } else {
        row = inserted
      }
    }
    ...
    if (!row) {
      ...
      logger.error('No usage row available after insert attempt', { userId })
    ...
    if (unitsUsed > USAGE_CONFIG.monthlyUnitCap) {
      const e = new Error('Usage limit reached for current billing period')
    ...
    // Propagate hard limit errors, but swallow others
    if (err.code === 'USAGE_LIMIT_REACHED' || err.code === 'NO_SUBSCRIPTION') {
      throw err
    }
