...
// ✅ NEW: Plan-specific limits
const USAGE_CONFIG = {
  business: {
    monthlyUnitCap: 175,  // $50 plan - 85% margin at full usage
    textWeight: 1,
    imageWeight: 2,
  },
  enterprise: {
    monthlyUnitCap: 900,  // $100 plan - 85% margin at full usage
    textWeight: 1,
    imageWeight: 2,
  }
}
...
export async function logUsageForAnalytics(payload) {
  ...
  const planConfig = USAGE_CONFIG[planName] || USAGE_CONFIG.business
  const monthlyUnitCap = planConfig.monthlyUnitCap
  const textWeight = planConfig.textWeight
  const imageWeight = planConfig.imageWeight
  ...
  try {
    ...
    const unitsUsed =
      nextTextCount * textWeight +
      nextImageCount * imageWeight

    if (unitsUsed > monthlyUnitCap) {
      const e = new Error(
    ...
    if (updateError) {
      ...
      const remainingUnits = monthlyUnitCap - unitsUsed
      ...
      return {
        ...
        unlimited: false,
        remainingUnits,
        usagePercent: Math.round((unitsUsed / monthlyUnitCap) * 100),
      }
    }
  } catch (err) {
    if (
