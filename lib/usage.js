...
// Cache schema lookups so we don't hammer information_schema on every request
let usageCountersSchema = null
let usageEventsTableExists = null

async function fetchTableColumns(supabaseAdmin, tableName) {
  try {
    ...
    if (error) {
      logger.warn('Schema lookup failed', { table: tableName, error: error.message })
      return null
    }

    return new Set((data || []).map((c) => c.column_name))
  } catch (err) {
    logger.warn('Schema lookup exception', { table: tableName, error: err?.message })
    return null
  }
}
...
async function ensureUsageCountersSchema(supabaseAdmin) {
  ...
  const columns = await fetchTableColumns(supabaseAdmin, 'usage_counters')

  usageCountersSchema = {
    exists: Boolean(columns && columns.size > 0),
    hasLastDuration: Boolean(columns?.has('last_duration_ms')),
    hasLastMode: Boolean(columns?.has('last_mode')),
    hasLastSuccess: Boolean(columns?.has('last_success')),
  }

  if (!usageCountersSchema.exists) {
    logger.warn('usage_counters table missing - usage logging will be skipped')
  } else {
    const missing = []
    if (!usageCountersSchema.hasLastDuration) missing.push('last_duration_ms')
    if (!usageCountersSchema.hasLastMode) missing.push('last_mode')
    if (!usageCountersSchema.hasLastSuccess) missing.push('last_success')

    if (missing.length > 0) {
      logger.warn('usage_counters missing expected columns; writing fallback payload', {
        missing,
      })
    }
  }
...
async function ensureUsageEventsTable(supabaseAdmin) {
  ...
  try {
    ...
    if (error) {
      logger.warn('Usage events table lookup failed', { error: error.message })
      usageEventsTableExists = false
      return usageEventsTableExists
    }

    usageEventsTableExists = Boolean(data?.table_name)
    if (!usageEventsTableExists) {
      logger.warn('usage_events table missing - detailed usage logs disabled')
    }

    return usageEventsTableExists
  } catch (err) {
    logger.warn('Usage events table lookup exception', { error: err?.message })
    usageEventsTableExists = false
    return usageEventsTableExists
  }
}
