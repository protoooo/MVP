// lib/usage.js - FIXED: Secure trial verification with Stripe payment method checks
// Extended trial conversion grace period + Stripe verification + Payment method enforcement
...
/**
 * ✅ FIXED: Secure trial verification with payment method enforcement
 * - Always verifies Stripe subscription status during trial
 * - Enforces payment method requirement before trial expiration
 * - Extended grace period only for active payment processing
 */
async function verifyTrialExpiration(sub, userId) {
  ...
  // ✅ FIX: Check if user has Stripe subscription with payment method
  if (!sub.stripe_subscription_id) {
    ...
    // Trial still active, but no payment method - warn user
    const hoursLeft = Math.round((trialEnd - now) / (1000 * 60 * 60))
    logger.info('Trial access granted (no payment method yet)', { userId, hoursLeft })
    return { valid: true, subscription: sub, warning: 'NO_PAYMENT_METHOD' }
  }
  
  // ✅ FIX: Always verify with Stripe during trial period
  try {
    ...
    if (trialEnd >= now) {
      ...
      if (!hasPaymentMethod) {
        // Trial active but no payment method - warn but allow
        logger.warn('Trial active without payment method', { 
          userId, 
          hoursLeft,
      ...
      logger.info('Trial access granted (payment method attached)', { userId, hoursLeft })
      return { valid: true, subscription: sub }
    }
    
    // ✅ FIX: Trial has expired - enforce payment method requirement
    if (!hasPaymentMethod) {
    ...
    // Payment method exists - check conversion status
    ...
    if (stripeSubscription.latest_invoice?.payment_intent?.status === 'processing') {
      const GRACE_PERIOD_MINUTES = 60 // ✅ 60 minutes grace period
    ...
    // If Stripe API fails, use limited fallback grace period
    ...
    // ✅ FIX: Shorter fallback grace period when Stripe is unreachable
    const FALLBACK_GRACE_MINUTES = 15 // ✅ Only 15 minutes if Stripe is down
    const gracePeriodEnd = new Date(trialEnd.getTime() + FALLBACK_GRACE_MINUTES * 60 * 1000)
    
    if (now < gracePeriodEnd) {
      logger.warn('Using fallback grace period (Stripe unreachable)', { 
        userId,
